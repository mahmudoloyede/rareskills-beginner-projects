...
contract StakerTest is Test {
    ...
    function testStake() public {
        ...
        uint256 stakerCloudBal = cloudToken.balanceOf(address(staker));
        uint256 thisCloudBal = cloudToken.balanceOf(address(this));
        uint256 totalStaked = staker.totalStaked();
        (uint256 amount, uint256 stakeDate, bool collectedReward) = staker.stakes(address(this));
        assertEq(amount, 0);
    ...
    function testCollectStakeAndRewardWhenStakedForAtleast7Days() public {
        ...
        uint256 thisCloudBal = cloudToken.balanceOf(address(this));
        staker.collectStakeAndReward();
        (uint256 stakedAmount,, bool collected) = staker.stakes(address(this));
        uint256 reward = stakedAmount * 1_000_000 ether / staker.totalStaked();
